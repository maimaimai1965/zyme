...
public class TokenField extends CustomField implements Container.Editor {
    ...
    public TokenField(Layout lo) {
        setStyleName(STYLE_TOKENFIELD);

        cb.setImmediate(true);
        cb.setNewItemsAllowed(true);
        cb.setNullSelectionAllowed(false);
        cb.addListener((ValueChangeListener) event -> {
        ...
        // container.
        cb.setNewItemHandler((NewItemHandler) tokenId -> {
            if (isReadOnly()) {
    ...
    private void addTokenButton(final Object val) {
        ...
        b.addClickListener((ClickListener) clickEvent -> onTokenClick(val));
        buttons.put(val, b);
    ...
    public void setReadOnly(boolean readOnly) {
        ...
        for (Button b : buttons.values()) {
            ((ExtendedButton)b).setReadOnly(readOnly);
        }
