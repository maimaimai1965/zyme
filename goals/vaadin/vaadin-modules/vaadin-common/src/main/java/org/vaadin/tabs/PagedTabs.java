...
public class PagedTabs extends Composite<Tabs> {
    ...
    public PagedTabs(HasComponents componentContainer) {
        ...
        getContent().addSelectedChangeListener(event -> {
            if (event.getSelectedTab() != null) {
                ...
                event.getSelectedTab().setEnabled(true);
                additionalSelectEvent(tabToComponent.get(event.getSelectedTab()));
            }
        });
        getContent().addThemeVariants(TabsVariant.LUMO_MINIMAL);
    }

    public void additionalSelectEvent(Component component) {
    ...
    public Tab add(String tabText, Component component, boolean closable) {
        ...
        ((FlexComponent)componentContainer).setFlexGrow(1, component);
        ((FlexComponent)componentContainer).setAlignSelf(FlexComponent.Alignment.CENTER, component);
//        ((FlexComponent)componentContainer).setAlignItems(FlexComponent.Alignment.CENTER);
        ((FlexComponent)componentContainer).setJustifyContentMode(FlexComponent.JustifyContentMode.CENTER);
    ...
    public void setSelectedTabByIndex(int selectedIndex){
        getContent().setSelectedIndex(selectedIndex);
    }
