...
public class ShortField extends AbstractNumberField<ShortField, Short> {
     ...
     */
    public ShortField() {
        this(new ShortField.Formatter(), true);
    }
     ...
     */
    private ShortField(ShortField.Formatter formatter, boolean isInitialValueOptional) {
        super(formatter::parse, formatter, Short.MIN_VALUE,
                Short.MAX_VALUE, isInitialValueOptional);
    }
     ...
     *            the maximum length
     *
     * @deprecated Not supported by ShortField (as it's built on
    ...
    private static class Formatter
            implements SerializableFunction<Short, String> {
        ...
        private Formatter() {
            decimalFormat.setMaximumFractionDigits(Short.MAX_VALUE);
        }

        @Override
        public String apply(Short valueFromModel) {
            return valueFromModel == null ? ""
                    : decimalFormat.format(valueFromModel.shortValue());
        }

        private Short parse(String valueFromClient) {
            try {
                return valueFromClient == null || valueFromClient.isEmpty()
                        ? null
                        : decimalFormat.parse(valueFromClient).shortValue();
